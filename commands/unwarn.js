...
module.exports = {
  ...
  async execute(message) {
    ...
    if (!message.member.roles.cache.has(STAFF_ROLE_ID)) return message.reply('❌ Permission refusée.');
    if (!target) return message.reply('❌ Mentionne un utilisateur.');
    if (target.roles.cache.has(STAFF_ROLE_ID)) return message.reply('❌ Pas possible sur un autre staff.');
